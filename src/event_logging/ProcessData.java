...
public class ProcessData {
	...
	protected static String processPrice(Node price) {
		
		if (price.getNodeType() == Node.ELEMENT_NODE) {

			Element eElement = (Element) price;
			String priceData = "Price: ";
			
			priceData += ("Destination - " + eElement.getElementsByTagName("to").item(0).getTextContent()+ ", ");
			priceData += ("Origin - " + eElement.getElementsByTagName("from").item(0).getTextContent()+ ", ");
			priceData += ("Priority - " + eElement.getElementsByTagName("priority").item(0).getTextContent()+ ", ");
			priceData += ("Volume Cost - " + eElement.getElementsByTagName("volumecost").item(0).getTextContent()+ ", ");
			priceData += ("Weight Cost - " + eElement.getElementsByTagName("weightcost").item(0).getTextContent()+ "\n ");
	...
	protected static String processMail(Node mail) {
		...
		if (mail.getNodeType() == Node.ELEMENT_NODE) {
			...
			//mailData += ("Day - " + eElement.getElementsByTagName("day").item(0).getTextContent() + ", ");
			mailData += ("Destination - " + eElement.getElementsByTagName("destination").item(0).getTextContent()+ ", ");
			mailData += ("Origin - " + eElement.getElementsByTagName("origin").item(0).getTextContent()+ ", ");
	...
	protected static String processRoute(Node route) {
		...
		if (route.getNodeType() == Node.ELEMENT_NODE) {
			...
			routeData += ("Destination - " + eElement.getElementsByTagName("destination").item(0).getTextContent()+ ", ");
			routeData += ("Origin - " + eElement.getElementsByTagName("origin").item(0).getTextContent()+ ", ");
